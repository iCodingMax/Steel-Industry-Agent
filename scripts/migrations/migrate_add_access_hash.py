...
并为已有应用生成唯一的access_hash值（16位随机十六进制）
"""
import os
import sys
import asyncio
...
# 定位 backend/.env（脚本位于 scripts/migrations/，backend 在项目根下）
...
async def main():
    ...
    try:
        ...
        pg_password = os.getenv("PG_PASSWORD", "123456aA")
        pg_host = os.getenv("PG_HOST", "172.1.8.23")
        ...
        if result:
            ...
            await conn.execute("""
                ...
                ADD COLUMN access_hash VARCHAR(16)
            """)
            await conn.execute("""
                ALTER TABLE applications 
